 ...
 *
 *         Geometries can be created from from Shapefiles. Support for
 *         GTVectorBindings is almost ready, but not complete yet.
 *
 *         GeometryCollections: Each Geometry will be transformed to a JSON
 *         geometry separately, they will be put in an array [geometry,
 *         geometry, geometry]
 *
 *         MultiLineStrings and MultiPolygons: These geometries are not defined
 *         in http://help.arcgis.com/en/webapi/javascript/
 *         arcgis/help/jsapi_start.htm, only Point, Polyline, Polygon and
 *         MultiPoint are defined. MultiLineStrings and MultiPolygons are (by
 *         default) transformed to normal Polygons and Polylines, having
 *         multiple "paths"/"rings". As an alternative, MultiLineStrings and
 *         MultiPolygons can be handled as several linestrings / polygons in an
 *         array (MultiLineString = [linestring, linestring, linestring], etc.).
 *         For this, set "multiGeometriesToArray" to false.
 *
 */
...
public class JSONGeometryGenerator extends AbstractGenerator {
    ...
    boolean isShapefile = false; // needed for a workaround: if the input is gml
                                 // without a specified crs,
                                 // the coordinates usually are swapped,
                                 // compared to the shapefile

    boolean multiGeometriesToArray = false; // if you prefer the multipolygons
                                            // or multilinestrings as an
                                            // array [polygon, polygon,
                                            // polygon], set this to true. If
                                            // you prefer
                                            // them as multiple rings/paths in
                                            // one linestring or polygon, set it
                                            // to
                                            // false
        ...
        if (data instanceof GenericFileDataWithGTBinding) {
            ...
            try {
                ...
                throw new IOException(
                        "The data passed from the algorithm to the generator is a file, but no shapefile");
            }
     ...
     *
     * The feature that is passed to this method is transformed to a JSON string
    ...
    private String transformPointToJsonPoint(Point p) {
        ...
        /*
         * Points are like this: {"x": -122.65, "y": 45.53, "spatialReference":
         * {" wkid": 4326 } } We will assemble it in five parts: (1) { (2) "x":
         * -122.65, (3) "y": 45.53, (4) "spatialReference": {" wkid": 4326 } (5)
         * }
         */
        ...
        /*
         * In jsongeometry, x stands for EAST/WEST coordinate, y for
         * NORTH/SOUTH.
         */
    ...
    private String transformMultiPointToJsonMultiPoint(MultiPoint multipoint) {
         ...
         * "spatialReference":({" wkid":4326 })} (1) {"points":[ (2)
         * [coords],[coords],[coords], (4) ] (5)
         * "spatialReference":{" wkid":4326 } (6) }
         */
        ...
        for (int i = 0; i < multipoint.getNumGeometries(); i++) {
            ...
            // TODO There should not be this difference!! I think the error is
            // that
            // x and y were wrongly given to GML input!
            double eastwest;
            double northsouth;
            ...
            /*
             * In JSONGEOMETRY, x stands for EAST/WEST coordinate, y for
             * NORTH/SOUTH. Lines are like this [eastwest, northsouth]
             */
    ...
    private String transformLineStringToJsonLineString(LineString linestring) {
         ...
         * "spatialReference":{"wkid":4326}} For a simple line, we put only one
         * path. We will assemble it in several parts: (a) {"paths":[ (b) one
         * path, like this: [ [coords],[coords],[coords] ] (it has square
         * brackets) (c) ], (d) "spatialReference":{"wkid":4326} (e) }
         */

        // (1) make path
        String aPath = linestringToJsonPath(linestring); // like this: [
    ...
    private String transformMultiLineStringToJsonLineString(MultiLineString multilinestring) {
         ...
         * Lines are like this: {"paths":[[[-122.68,45.53],
         * [-122.58,45.55],[-122.57,45.58],[-122.53,45.6]]],
         * "spatialReference":{"wkid":4326}} For a multi line, we put several
         * paths. We will assemble it in several parts: (a) {"paths":[ (b)
         * several paths, comma separated, each has square brackets: (b) [
         * [coords],[coords],[coords] ] <== this is one path, with square
         * brackets! (b) [ [coords],[coords],[coords] ], <== this is one path,
         * with square brackets! (b) [ [coords],[coords],[coords] ], <== this is
         * one path, with square brackets! (c) ], (d)
    ...
    private String transformMultiLineStringToJsonLineStringArray(MultiLineString multilinestring) {
        ...
        /*
         * For a multi line, we make several simple lines and put them into an
         * array
         */
    ...
    // Helpers
    private String assembleLine(String aPathOrSeveralPaths,
            Geometry geometry) {
    ...
    private String linestringToJsonPath(LineString ls) {
        ...
        /*
         * A path is like this: [ [-122.68,45.53],
         * [-122.58,45.55],[-122.57,45.58],[-122.53,45.6] ]
         *
    ...
    private String transformPolygonToJsonPolygon(Polygon polygon) {
         ...
         * {"rings":[[[-122.63,45.52],[-122.57,45.53],[-122.52,45.50],[-122.49,
         * 45.48],
         * [-122.64,45.49],[-122.63,45.52],[-122.63,45.52]]],"spatialReference":
         * {" wkid":4326 }}
         *
         * For a simple polygon, we take only one ring:
         *
         * (a) {"rings":[ (b) a ring, like this: [[coords],[coords],[coords]]
         * <== this is one ring, with square brackets (c) ], (d)
         * "spatialReference":{" wkid":4326 } (e) }
         */
    ...
    private String transformMultiPolygonToJsonPolygon(MultiPolygon multipolygon) {
         ...
         * Lines are like this: {"paths":[[[-122.68,45.53],
         * [-122.58,45.55],[-122.57,45.58],[-122.53,45.6]]],
         * "spatialReference":{"wkid":4326}} For a multi polygon, we put several
         * paths. We will assemble it in several parts: (a) {"paths":[ (b)
         * several paths, comma separated, each has square brackets: (b) [
         * [coords],[coords],[coords] ] <== this is one path, with square
         * brackets! (b) [ [coords],[coords],[coords] ], <== this is one path,
    ...
    private String transformMultiPolygonToJsonPolygonArray(MultiPolygon multipolygon) {
        ...
        /*
         * For a multi polygon, we make several simple polygons and put them
         * into an array
         */
    ...
    private String getPathFromPolygon(Polygon polygon) {
        ...
        if (holes < 0) {
            LOGGER.info("The polygon that is being transformed to an json geometry "
                    + "has holes. Holes are not accepted. Only the outer ring will be " + "transformed.");
        }
    ...
    private String assemblePolygon(String aRingOrSeveralRings,
            Geometry geometry) {

        /*
         * Polygons are like this: (a) {"rings":[ (b) a ring or several rings
         * (comma separated) (c) ], (d) "spatialReference":{" wkid":4326 } (e) }
         */
