...
public class RDataTypeRegistry {
    ...
    public void register(RDataType type){
    	rDataTypeKeys.put(type.getKey(), type);
        ...
        if ( !containsKey(type.getProcessKey()))
            rDataTypeAlias.put(type.getProcessKey(),type);
        else
    ...
    public boolean containsKey(String key){
    	return rDataTypeKeys.containsKey(key) || rDataTypeAlias.containsKey(key);
    }
    ...
    public RTypeDefinition getType(String key) throws RAnnotationException {
        RTypeDefinition out = rDataTypeKeys.get(key);
        if(out==null)
        	out = rDataTypeAlias.get(key);
        if(out==null)
        	out = customDataTypes.get(key);
        if (out == null)
	...
	private String addTabbs(String s, int nmax){
		int n = nmax-s.length();
		String out="";
		for (int i = 0; i < n; i++) {
			out+=" ";
	...
	public void register(CustomDataType type) {
		customDataTypes.put(type.getKey(), type);
		
	}
	
	/**
	 * Deletes all registered custom type definitions
	 * (Useful for instance, if the config file was changed)
	 */
