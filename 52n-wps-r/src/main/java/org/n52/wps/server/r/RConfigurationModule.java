...
public class RConfigurationModule extends ClassKnowingModule {
    ...
    private ConfigurationEntry<Boolean> enableBatchStartEntry = new BooleanConfigurationEntry(enableBatchStartKey,
            "Enable Batch Start", "Try to start Rserve on the local machine", false, false);

    private ConfigurationEntry<String> datatypeConfigEntry =
            new StringConfigurationEntry(datatypeConfigKey, "Custom data type mappings",
                    "Location of a config file were you may add costum data types that WPS4R should handle (see below)",
                    false, "R/R_Datatype.conf");

    private ConfigurationEntry<String> wdStrategyEntry =
            new StringConfigurationEntry(wdStrategyKey, "Working Directory Strategy",
                    "Influences WPS4R on choosing the R working directory for each process run", false, "default");

    private ConfigurationEntry<String> wdNameEntry = new StringConfigurationEntry(wdNameKey, "Working Directory",
            "The path for the manually set work directory or base directory in conjuction with the strategy 'manualbasedir'",
            false, "wps4r_working_dir");

    private ConfigurationEntry<String> resourceDirectoryEntry = new StringConfigurationEntry(resourceDirectoryKey,
            "Resource Directory",
            "The (relative) path to a directory with resources that can be requested in scripts (default: 'R/resources')",
            false, "R/resources");

    private ConfigurationEntry<String> scriptDirectoryEntry = new StringConfigurationEntry(scriptDirectoryKey,
            "Script Directory",
            "One (or several delimited by ';') folder where R scripts are located and loaded from (default: 'R/scripts')",
            false, "R/scripts");

    private ConfigurationEntry<String> rServeHostEntry = new StringConfigurationEntry(rServeHostKey, "RServe Host",
            "The RServe Host URL (default: 'localhost')", false, "localhost");

    private ConfigurationEntry<String> rServePortEntry = new StringConfigurationEntry(rServePortKey, "RServe Port",
            "The RServe Port (default: '6311')", false, "6311");

    private ConfigurationEntry<String> rServeUserEntry = new StringConfigurationEntry(rServeUserKey, "RServe User",
            "Username for Rserve authentication (only if Rserve requires login)", false, "changeme");

    private ConfigurationEntry<String> rServePasswordEntry = new StringConfigurationEntry(rServePasswordKey,
            "RServe Password", "Password for Rserve authentication (only if Rserve requires login)", false, "changeme");

    private ConfigurationEntry<String> rServeUtilsScriptsDirectoryEntry = new StringConfigurationEntry(
            rServeUtilsScriptDirectoryKey, "RServe Utils Scripts",
            ...
            false, "R/utils;org/n52/wps/R/unzipRenameFile.R;org/n52/wps/R/wpsStatus.R");

    private ConfigurationEntry<Boolean> cacheProcessesEntry = new BooleanConfigurationEntry(cacheProcessesKey,
            "Cache Processes",
            "If deactivated, the scripts files are loaded every time the algorithm is called, for development (default: true)",
            false, true);

    private ConfigurationEntry<String> sessionMemoryLimitEntry = new StringConfigurationEntry(sessionMemoryLimitKey,
            "Session Memory Limit", "Set the R session memory limit (default: 1000)", false, "1000");

    private ConfigurationEntry<Boolean> resourceDownloadEnabledEntry = new BooleanConfigurationEntry(
            resourceDownloadEnabledKey, "Enable resource download", "Allows to download R resources", false, true);

    private ConfigurationEntry<Boolean> importDownloadEnabledEntry = new BooleanConfigurationEntry(
            importDownloadEnabledKey, "Enable imports download", "Allows to download R imports", false, true);

    private ConfigurationEntry<Boolean> scriptDownloadEnabledEntry = new BooleanConfigurationEntry(
            scriptDownloadEnabledKey, "Enable script download", "Allows to download R scripts", false, true);

    private ConfigurationEntry<Boolean> sessionInfoDownloadEnabledEntry =
            new BooleanConfigurationEntry(sessionInfoDownloadEnabledKey, "Enable session info download",
                    "Allows to download R session info", false, true);
    ...
    private List<? extends ConfigurationEntry<?>> configurationEntries = Arrays.asList(enableBatchStartEntry,
            datatypeConfigEntry, wdStrategyEntry, wdNameEntry, resourceDirectoryEntry, scriptDirectoryEntry,
            rServeHostEntry, rServePortEntry, rServeUserEntry, rServePasswordEntry, rServeUtilsScriptsDirectoryEntry,
            cacheProcessesEntry, sessionMemoryLimitEntry, resourceDownloadEnabledEntry, importDownloadEnabledEntry,
            scriptDownloadEnabledEntry, sessionInfoDownloadEnabledEntry);
    ...
    public Boolean isEnableBatchStart() {
        return isNullOrEmpty(enableBatchStart, enableBatchStartEntry) ? enableBatchStartEntry.getValue()
                : enableBatchStart;
    }

    @ConfigurationKey(
            key = enableBatchStartKey)
    public void setEnableBatchStart(boolean enableBatchStart) {
        this.enableBatchStart = enableBatchStart;
    }

    public String getDatatypeConfig() {
        return isNullOrEmpty(datatypeConfig, datatypeConfigEntry) ? datatypeConfigEntry.getValue() : datatypeConfig;
    }

    @ConfigurationKey(
            key = datatypeConfigKey)
    public void setDatatypeConfig(String datatypeConfig) {
        this.datatypeConfig = datatypeConfig;
    }

    public String getWdStrategy() {
        return isNullOrEmpty(wdStrategy, wdStrategyEntry) ? wdStrategyEntry.getValue() : wdStrategy;
    }
    ...
    public String getWdName() {
        return isNullOrEmpty(wdName, wdNameEntry) ? wdNameEntry.getValue() : wdName;
    }
    ...
    public String getResourceDirectory() {
        return isNullOrEmpty(resourceDirectory, resourceDirectoryEntry) ? resourceDirectoryEntry.getValue()
                : resourceDirectory;
    }

    @ConfigurationKey(
            key = resourceDirectoryKey)
    public void setResourceDirectory(String resourceDirectory) {
        this.resourceDirectory = resourceDirectory;
    }

    public String getScriptDirectory() {
        return isNullOrEmpty(scriptDirectory, scriptDirectoryEntry) ? scriptDirectoryEntry.getValue() : scriptDirectory;
    }

    @ConfigurationKey(
            key = scriptDirectoryKey)
    public void setScriptDirectory(String scriptDirectory) {
        this.scriptDirectory = scriptDirectory;
    }

    public String getRServeHost() {
        return isNullOrEmpty(rServeHost, rServeHostEntry) ? rServeHostEntry.getValue() : rServeHost;
    }
    ...
    public String getRServePort() {
        return isNullOrEmpty(rServePort, rServePortEntry) ? rServePortEntry.getValue() : rServePort;
    }
    ...
    public void setRServePort(String rServePort) {
        try {
            ...
            LOGGER.warn("Config variable {} does not contain a parseble integer. Using default port 6311.",
                    rServePortKey, e);
            rServePort = "6311";
        }
    ...
    public String getRServeUser() {
        return isNullOrEmpty(rServeUser, rServeUserEntry) ? rServeUserEntry.getValue() : rServeUser;
    }
    ...
    public String getrServePassword() {
        return isNullOrEmpty(rServePassword, rServePasswordEntry) ? rServePasswordEntry.getValue() : rServePassword;
    }

    @ConfigurationKey(
            key = rServePasswordKey)
    ...
    @ConfigurationKey(
            key = rServeUtilsScriptDirectoryKey)
    ...
    public Boolean isCacheProcesses() {
        return isNullOrEmpty(cacheProcesses, cacheProcessesEntry) ? cacheProcessesEntry.getValue() : cacheProcesses;
    }

    @ConfigurationKey(
            key = cacheProcessesKey)
    ...
    public String getSessionMemoryLimit() {
        return isNullOrEmpty(sessionMemoryLimit, sessionMemoryLimitEntry) ? sessionMemoryLimitEntry.getValue()
                : sessionMemoryLimit;
    }

    @ConfigurationKey(
            key = sessionMemoryLimitKey)
    ...
    @ConfigurationKey(
            key = resourceDownloadEnabledKey)
    ...
    @ConfigurationKey(
            key = sessionInfoDownloadEnabledKey)
