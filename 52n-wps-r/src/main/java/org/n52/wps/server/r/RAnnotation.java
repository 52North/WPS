 ...
 * 
 * Syntax in (raw) BNF:
 *  <RAnnotation> ::= <StartKey> <AttributeSequence> <EndKey>
 *  <StartKey> <Attributequence> ::= <RAnnotationTypeInstance>.getStartKey() <RAnnotationTypeInstance>.getAttributeSequence()
 *  <EndKey> ::= RSeparator.ANNOTATION_END.getKey()
 *  <AttributeSequence> ::= {<RAttributeInstance>.getKey() ATRIBUTE_VALUE_SEPARATOR} <Attributevalue> {ATTRIBUTE_SEPARATOR <RAttributeSequence>}
 * 
...
public class RAnnotation {
	 ...
	 * @param type
	 * @param attributeHash
	 * @throws IOException if AttributHash is not valid for any cause
	 * @throws RAnnotationException 
	 */
	public RAnnotation(RAnnotationType type,
			HashMap<RAttribute, String> attributeHash) throws IOException, RAnnotationException {
		super();
		this.type = type;
...
//	}
	...
	public static List<RAnnotation> filterAnnotations(List<RAnnotation> annotations, RAnnotationType type, RAttribute attribute, String value){
		...
		for(RAnnotation annotation:annotations){
			...
			if(type == null || annotation.getType()==type){
				...
				if(attribute == null || value == null || 
					annotation.getAttribute(attribute).equalsIgnoreCase(value)){
						out.add(annotation);
				}
			}
		}
	...
	/**
	 * 
	 * @param rClass - value referring to RAttribute.TYPE
	...
	/**
	 * Checks if the type - argument of an annotation refers to
	 * complex data
	 * @return 
	 * @throws RAnnotationException 
	 */
	...
	public boolean isComplex() throws RAnnotationException{
		return isComplex(
				this.getAttribute(RAttribute.TYPE)
		);
	}
	...
	/**
	 * Data types which are supported by scripts
	 * Note that every IData class must be parsed from an to are to be handled successful
	 * --> GenericRProcess
	 * TODO: restructure dependent classes & methods for new attributes
	 */
	public enum RDataType{
		...
		private RDataType(String key, String processKey,
				Class<? extends IData> iDataClass, boolean isComplex,
				String schema, String encoding) {
			this.key = key;
		...
		/**
		 * This method is important for parsers to request the meaning
		 * of a specific key
		 * @param key process keys and self defined short keys are recognized as dataType keys
		 * @return
		 * @throws RAnnotationException 
		 */
		public static RDataType getType(String key) throws RAnnotationException {
			RDataType out = rDataTypeKeys.get(key);
			if(out == null) throw new RAnnotationException("Invalid datatype key for r - script annotations: " +key);
			else return out;
		}
	...
	public enum RAnnotationType{
		...
		public RAttribute getAttribute(String key) throws RAnnotationException{
			...
			if(attributeLut.containsKey(key))
				return attributeLut.get(key);
			 	else throw new RAnnotationException(
					"Annotation for "+ this+" ("+ this.startKey +" ...) contains no key named: "+key+".");
		}
		...
		/**
		 * Checks if Annotation content is valid for process description
		 * and adds attributes / standard values if necessary
		 * @param key / value pairs given in the annotation from RSkript
		...
		public void validDescription(HashMap<RAttribute, String> keyValues) throws RAnnotationException{
			...
			try{
				if(keyValues.containsKey(RAttribute.MIN_OCCURS)){
					...
					if(keyValues.containsKey(RAttribute.DEFAULT_VALUE)&&
							!min.equals(0))
						throw new RAnnotationException("");
				}
			}catch(NumberFormatException e){
				throw new RAnnotationException(
				"Syntax Error in Annotation "+ this + " ("+ this.startKey +" ...), "
				+ "unable to parse Integer value from attribute " + RAttribute.MIN_OCCURS.getKey() 
			    + e.getMessage());
			}
			
			if(keyValues.containsKey(RAttribute.DEFAULT_VALUE)&&
					!keyValues.containsKey(RAttribute.MIN_OCCURS)){
	...
	public enum RAttribute{
		...
		/**
		 * @return true if attribute has to occur in Process description,
		 * if so, there has to be a standard value or a value in R Annotion given
		 */
