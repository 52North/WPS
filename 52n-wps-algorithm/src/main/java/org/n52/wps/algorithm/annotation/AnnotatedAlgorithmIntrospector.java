...
public class AnnotatedAlgorithmIntrospector {
    ...
    private void parseClass() {
        ...
        algorithmBuilder = AlgorithmDescriptor.builder(
                algorithm.identifier().length() > 0 ? algorithm.identifier() : algorithmClass.getCanonicalName());

        algorithmBuilder.title(algorithm.title()).abstrakt(algorithm.abstrakt()).version(algorithm.version())
                .storeSupported(algorithm.storeSupported()).statusSupported(algorithm.statusSupported());

        if (metadataAnnnotation != null) {
            String[] roles = metadataAnnnotation.roles();
            for (int i = 0; i < roles.length; i++) {
                algorithmBuilder.addMetadataDescriptor(MetadataDescriptor.builder().href(metadataAnnnotation.hrefs()[i])
                        .role(metadataAnnnotation.roles()[i]));
            }
        }
            ...
            if (method.isAnnotationPresent(PROCESS_PARSER.getSupportedAnnotation())) {
                ...
                if (executeMethodBinding != null) {
                    if (this.executeMethodBinding != null) {
                        ...
                        throw new RuntimeException("Multiple execute method bindings encountered for class "
