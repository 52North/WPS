...
public class GeneratorFactory {
    ...
    private GeneratorFactory(Map<String, ConfigurationModule> generatorMap) {
        ...
        org.n52.wps.commons.WPSConfig.getInstance().addPropertyChangeListener(
                org.n52.wps.commons.WPSConfig.WPSCONFIG_PROPERTY_EVENT_NAME, new PropertyChangeListener() {
                    public void propertyChange(final PropertyChangeEvent propertyChangeEvent) {
                        LOGGER.info(this.getClass().getName() + ": Received Property Change Event: "
                                + propertyChangeEvent.getPropertyName());
                        loadAllGenerators(
                                org.n52.wps.commons.WPSConfig.getInstance().getActiveRegisteredGeneratorModules());
                    }
                });
    }
    ...
    public static GeneratorFactory getInstance() {
        if (factory == null) {
            Map<String, ConfigurationModule> generatorMap =
                    WPSConfig.getInstance().getConfigurationManager().getConfigurationServices()
                            .getActiveConfigurationModulesByCategory(ConfigurationCategory.GENERATOR);
