...
public class GrassIOHandler {
     ...
     *            complex inputdata for the process
     * @param literalInputData
     *            literal inputdata for the process
     * @param outputID
    ...
    public IData executeGrassProcess(String processID,
        ...
        outputFileName = tmpDir + fileSeparator + "out" + UUID.randomUUID().toString().substring(0, 5) + "."
                + GenericFileDataConstants.mimeTypeFileTypeLUT().get(outputMimeType);

        boolean success = createInputTxt(processID, complexInputData, literalInputData, outputID, outputFileName,
                outputMimeType, outputSchema);
    ...
    private String getCommand() {
        ...
        if (command == null) {
            ...
            command = getPythonHome() + fileSeparator + pythonName + " " + grassModuleStarterHome + fileSeparator
                    + "GrassModuleStarter.py -f " + getInputTxtFilename() + " -l " + tmpDir + fileSeparator + uuid
                    + logFilename + " -o " + tmpDir + fileSeparator + stdOutFilename + " -e " + tmpDir + fileSeparator
                    + stdErrorFilename;
        }
    ...
    private String[] getEnvp() {
        ...
        if (envp == null) {
            ...
            if (!OS_Name.startsWith("Windows")) {
                ...
                envp = new String[] { "GDAL_DATA=" + grassHome + fileSeparator + "etc" + fileSeparator + "ogr_csv",
                ...
                envp = new String[] { "APPDATA=" + appDataDir, "GDAL_DATA=" + grassHome + "\\etc\\ogr_csv",
                        "PWD=" + grassHome,
                        ...
                        // "PYTHONPATH=" + grassHome +
                        // "\\etc\\python;c:\\python25",
                        // "PYTHONHOME=C:\\OSGeo4W\\apps\\Python25",
                        "PYTHONHOME=" + pythonHome, "GRASS_CONFIG_DIR=.grass7", "GRASS_GNUPLOT=gnuplot -persist",
                        "GRASS_PAGER=less", "GRASS_PYTHON=python", "GRASS_SH=/bin/sh", "GRASS_VERSION=7.0.svn",
                        "SystemRoot=" + System.getenv("SystemRoot"), "WINGISBASE=" + grassHome };
            }
        }
    ...
    private String getSystemBlock() {
        if (systemBlock == null) {
            systemBlock = "[System]" + lineSeparator + " WorkDir=" + WORKDIR + lineSeparator + " OutputDir=" + OUTPUTDIR
                    + lineSeparator;
        }
        return systemBlock;
    }
    ...
    private String getGrassBlock() {
        if (grassBlock == null) {
            grassBlock = "[GRASS]" + lineSeparator + " GISBASE=" + grassHome + lineSeparator + " GRASS_ADDON_PATH="
                    + GRASS_ADDON_PATH + lineSeparator + " GRASS_VERSION=7.0.svn" + lineSeparator + " Module="
                    + PROCESS_IDENTIFIER + lineSeparator + " LOCATION=" + lineSeparator + " LinkInput=FALSE"
                    + lineSeparator + " IgnoreProjection=FALSE" + lineSeparator + " UseXYLocation=FALSE"
                    + lineSeparator;
        }
        return grassBlock;
    }
    ...
    private String getComplexInputDataBlock() {
        if (complexInputDataBlock == null) {
            complexInputDataBlock = "[ComplexData]" + lineSeparator + " Identifier=" + INPUT_IDENTIFIER + lineSeparator
                    + " MaxOccurs=1" + lineSeparator + " PathToFile=" + INPUT_PATH + lineSeparator + " MimeType="
                    + MIMETYPE + lineSeparator + " Encoding=" + ENCODING + lineSeparator + " Schema=" + SCHEMA
                    + lineSeparator;
        }
    ...
    private String getLiteralInputDataBlock() {
        if (literalInputDataBlock == null) {
            literalInputDataBlock = "[LiteralData]" + lineSeparator + " Identifier=" + INPUT_IDENTIFIER + lineSeparator
                    + " DataType=" + DATA_TYPE + lineSeparator + " Value=" + VALUE + lineSeparator;
        }
    ...
    private String getOutputDataBlock() {
        if (outputDataBlock == null) {
            outputDataBlock = "[ComplexOutput]" + lineSeparator + " Identifier=" + OUTPUT_IDENTIFIER + lineSeparator
                    + " PathToFile=" + OUTPUT_PATH + lineSeparator + " MimeType=" + MIMETYPE + lineSeparator
                    + " Encoding=" + ENCODING + lineSeparator + " Schema=" + SCHEMA + lineSeparator;
        }
    ...
    private boolean createInputTxt(String processID,
        ...
        try {
            ...
            for (String key : complexInputData.keySet()) {
                ...
                for (IData data : dataList) {
                    ...
                    if (mimetype.equals(GenericFileDataConstants.MIME_TYPE_TIFF)) {
                        ...
                        tmpBlock = getComplexInputDataBlock().replace(MIMETYPE,
                    ...
                    String filename =
                            ((GenericFileDataWithGTBinding) data).getPayload().getBaseFile(true).getAbsolutePath();
            ...
            if (outputMimeType.equals(GenericFileDataConstants.MIME_TYPE_PLAIN_TEXT)) {
                ...
                tmpBlock = tmpBlock.replace(ENCODING, "");
                tmpBlock = tmpBlock.replace(SCHEMA, "");
            } else if (outputMimeType.equals(GenericFileDataConstants.MIME_TYPE_TIFF)
                    || outputMimeType.equals(GenericFileDataConstants.MIME_TYPE_GEOTIFF)) {
                ...
                tmpBlock = tmpBlock.replace(ENCODING, "UTF-8");
                tmpBlock = tmpBlock.replace(SCHEMA, "http://schemas.opengis.net/gml/3.1.0/polygon.xsd");// TODO
                                                                                                        // change
                                                                                                        // to
                                                                                                        // gml2.1.2?!
