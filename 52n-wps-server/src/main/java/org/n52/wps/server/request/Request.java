...
abstract public class Request implements Callable<Response> {
     ...
     * @throws ExceptionReport
     *             if the key is required but is not present
     */
    public static String getMapValue(String key,
            CaseInsensitiveMap map,
            boolean required) throws ExceptionReport {
        if (map.containsKey(key)) {
            ...
            throw new ExceptionReport("Parameter <" + key + "> not specified.", ExceptionReport.MISSING_PARAMETER_VALUE,
                    key);
        }
    }
     ...
     *            a String array of supported values
     * @return The value of the key-value pair
     * @throws ExceptionReport
     *             if the key is required but is not present
     */
    public static String getMapValue(String key,
            CaseInsensitiveMap map,
            boolean required,
            String[] supportedValues) throws ExceptionReport {
        if (map.containsKey(key)) {
     ...
     *             if the key is required but is not present
     */
    public static String[] getMapArray(String key,
            CaseInsensitiveMap map,
            boolean required) throws ExceptionReport {
        if (map.containsKey(key)) {
     ...
     *             if the key is required but is not present
     */
    protected String[] getMapArray(String key,
            boolean required) throws ExceptionReport {
     ...
     * @throws ExceptionReport
     *             if the mandatory version is not present
     */
     ...
     * @param mandatory
     *            If the version must be part of the request
     * @return True if the required version matches, False otherwise.
     * @throws ExceptionReport
     *             if the mandatory version is not present
     */
    public boolean requireVersion(List<String> supportedVersions,
            boolean mandatory) throws ExceptionReport {
    ...
    /**
     * Checks, if the language is supported by the WPS. The language parameter
     * is optional, however, if a wrong language is requested, an
     * ExceptionReport has to be returned to the client.
     *
     ...
     * @throws ExceptionReport
     *             If a wrong language is requested, this ExceptionReport will
    ...
    public static void checkLanguageSupported(String language) throws ExceptionReport {
        ...
        throw new ExceptionReport("The requested language " + language + " is not supported",
