...
public class ExecuteResponseBuilderV200 implements ExecuteResponseBuilder {
    ...
    public ExecuteResponseBuilderV200(ExecuteRequestV200 request) throws ExceptionReport {
        ...
        XMLBeansHelper.addSchemaLocationToXMLObject(resultDoc,
    ...
    public void update() throws ExceptionReport {
        ...
        if (statusInfoDoc.getStatusInfo().getStatus().equals(Status.Succeeded.toString())) {
            ...
            if (request.isRawData()) {
                ...
                XMLBeansHelper.addSchemaLocationToXMLObject(statusInfoDoc,
                        "http://www.opengis.net/wps/2.0 http://schemas.opengis.net/wps/2.0/wps.xsd");
                DatabaseFactory.getDatabase().insertResponse(request.getUniqueId().toString(),
                        statusInfoDoc.newInputStream(XMLBeansHelper.getXmlOptions()));
                ...
                if (desc.getDataDescription() instanceof ComplexDataType) {
                    ...
                    generateLiteralDataOutput(id, resultDoc, true, reference, schema, mimeType, encoding,
                            desc.getTitleArray(0));
            ...
            for (int i = 0; i < request.getExecute().getOutputArray().length; i++) {
                ...
                if (desc.getDataDescription() instanceof ComplexDataType) {
                    ...
                    generateComplexDataOutput(responseID,
                            definition.getTransmission().equals(DataTransmissionModeType.REFERENCE), false, schema,
                            mimeType, encoding, desc.getTitleArray(0));
                } else if (desc.getDataDescription() instanceof LiteralDataType) {
                    String mimeType = null;
                    String schema = null;
                    String encoding = null;
                    ...
                    DomainMetadataType dataType = literalDataType.getLiteralDataDomainArray(0).getDataType();
                    String reference = dataType != null ? dataType.getReference() : null;
                    generateLiteralDataOutput(responseID, resultDoc, false, reference, schema, mimeType, encoding,
                            desc.getTitleArray(0));
                } else if (desc.getDataDescription() instanceof BoundingBoxData) {
    ...
    public String getMimeType(XmlObject definitionObject) {
        ...
        if (definitionObject instanceof OutputDefinitionType) {
            ...
            if (mimeType == null) {
                ...
                Format[] formats = outputDes.getDataDescription().getFormatArray();
                ...
                LOGGER.warn("Using default mime type: " + mimeType + " for output: " + inputID);
            }
        }
    ...
    private void generateComplexDataOutput(String responseID,
        ...
        if (rawData) {
            rawDataHandler =
                    new RawData(obj, responseID, schema, encoding, mimeType, this.identifier, superDescription);
        } else {
            OutputDataItem handler = new OutputDataItem(obj, responseID, schema, encoding, mimeType, title,
                    this.identifier, superDescription);
            if (asReference) {
    ...
    private void generateBBOXOutput(String responseID,
            ResultDocument res,
            boolean rawData,
            LanguageStringType title) throws ExceptionReport {
        ...
        if (rawData) {
            ...
            OutputDataItem handler =
                    new OutputDataItem(obj, responseID, null, null, null, title, this.identifier, superDescription);
    ...
    public InputStream getAsStream() throws ExceptionReport {
        ...
        if (request.getExecute().getMode().equals(ExecuteRequestType.Mode.SYNC)) {
            ...
            // if an error occurred, the status was set to failed and the
            // exception report was written to the database
    ...
    public void setStatus(XmlObject statusObject) {
        ...
        if (statusObject instanceof StatusInfo) {
            ...
            LOGGER.warn(String.format(
                    "XMLObject not of type \"net.opengis.wps.x20.StatusInfoDocument.StatusInfo\", but {}. Cannot not set status. ",
                    statusObject.getClass()));
        }
    }
