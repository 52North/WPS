...
public class InputHandler {
    ...
    private String algorithmIdentifier = null; // Needed to take care of
                                               // handling a conflict between
                                               // different parsers.
    ...
    private InputHandler(Builder builder) throws ExceptionReport {
        ...
        this.processDesc = (ProcessDescriptionType) RepositoryManagerSingletonWrapper.getInstance()
                .getProcessDescription(algorithmIdentifier).getProcessDescriptionType(WPSConfig.VERSION_100);
        this.processOffering = (ProcessOffering) RepositoryManagerSingletonWrapper.getInstance()
                .getProcessDescription(algorithmIdentifier).getProcessDescriptionType(WPSConfig.VERSION_200);
        ...
        if (inputsV100 != null) {
            ...
            for (DataInputType input : inputsV200) {
                ...
                if (input.getData() != null) {
                    ...
                    net.opengis.wps.x20.InputDescriptionType inputDescription =
                            XMLBeansHelper.findInputByID(inputId, processOffering.getProcess());
    ...
    Map<String, InterceptorInstance> resolveInputInterceptors(String algorithmClassName) {
        ...
        DataInputInterceptorImplementations annotation =
                clazz.getAnnotation(DataInputInterceptors.DataInputInterceptorImplementations.class);
        if (annotation != null) {
    ...
    ComplexDataDescriptionType getNonDefaultFormat(InputDescriptionType inputRefDesc,
            String dataMimeType,
            String dataSchema,
            String dataEncoding) {
        ...
        for (ComplexDataDescriptionType potentialFormat : formats) {
            ...
            if (potentialFormat.getMimeType().equalsIgnoreCase(dataMimeType)) {
                ...
                if (dataSchema != null && dataEncoding != null) {
                    if (dataSchema.equalsIgnoreCase(pFormatSchema) && dataEncoding.equalsIgnoreCase(pFormatEncoding)) {
                        return potentialFormat;
                    }
     ...
     *            the id of the input
     * @throws ExceptionReport
     *             If error occurred while parsing XML
     */
    protected void handleComplexData(InputType input,
            String inputId) throws ExceptionReport {
        String complexValue;
        ...
        if (inputReferenceDesc == null) {
            LOGGER.debug("Input cannot be found in description for " + processDesc.getIdentifier().getStringValue()
                    + "," + inputId);
        }
        ...
        if (data.isSetMimeType() && dataMimeType != null) {
            format = findComplexDataDescriptionType(inputReferenceDesc, dataMimeType, dataSchema, dataEncoding,
                ...
                if (StringUtils.isBlank(dataMimeType)) {
                    ...
                    if (data.isSetEncoding() && !data.isSetSchema()) {
                        ...
                        String defaultEncoding =
                                inputReferenceDesc.getComplexData().getDefault().getFormat().getEncoding();
                        ...
                        List<ComplexDataDescriptionType> foundEncodingList =
                                new ArrayList<ComplexDataDescriptionType>();
                        if (defaultEncoding.equalsIgnoreCase(data.getEncoding())) {
                            ...
                            // schema
                            List<ComplexDataDescriptionType> foundSchemaList =
                                    new ArrayList<ComplexDataDescriptionType>();
                            ...
                            if (defaultSchema != null && defaultSchema.equalsIgnoreCase(data.getSchema())) {
                                ...
                                for (ComplexDataDescriptionType tempFormat : formats) {
                                     ...
                                     * not by schema:
                                     *
                                     * old code: if(tempFormat.getEncoding().
                                     * equalsIgnoreCase(data.getSchema())){
        ...
        IParser parser = null;
        try {
            LOGGER.debug("Looking for matching Parser ..." + " schema: " + formatSchema + " mimeType: " + dataMimeType
    ...
    protected ComplexDataDescriptionType findComplexDataDescriptionType(InputDescriptionType inputReferenceDesc,
            String dataMimeType,
            String dataSchema,
            String dataEncoding,
     ...
     */
    private void handleComplexData(DataInputType input,
            String inputId) throws ExceptionReport {
        String complexValue;
        ...
        inputReferenceDesc = XMLBeansHelper.findInputByID(inputId, processOffering.getProcess());
        if (inputReferenceDesc == null) {
            LOGGER.debug("Input cannot be found in description for " + processDesc.getIdentifier().getStringValue()
                    + "," + inputId);
        }
        ...
        // overwrite with data format from request if appropriate
        if (data.isSetMimeType() && dataMimeType != null) {
            format = findFormat(inputReferenceDesc, dataMimeType, dataSchema, dataEncoding, potentialFormatSchema,
                    potentialFormatEncoding);
     ...
     *             parsing the XML.
     */
    private void handleComplexValueReference(DataInputType input) throws ExceptionReport {
        ...
        if (inputPD == null) { // check if there is a corresponding input
            ...
            LOGGER.debug("Input cannot be found in description for " + this.processDesc.getIdentifier().getStringValue()
                    + "," + inputID);
            throw new RuntimeException("Input cannot be found in description for "
        ...
        if (referenceData.isSetMimeType() && referenceData.getMimeType() != null) {
            ...
            if (defaultMimeType.equalsIgnoreCase(mimeType)) {
                ...
                if (referenceData.getSchema() != null && referenceData.getEncoding() != null) {
                    if (referenceData.getSchema().equalsIgnoreCase(potentialFormat.getSchema())
                            && referenceData.getEncoding().equalsIgnoreCase(potentialFormat.getEncoding())) {
                        canUseDefault = true;
                        format = potentialFormat;
                    }
            ...
            if (format == null) {
                throw new ExceptionReport(
                        "Possibly multiple or none matching generators found for the input data with id = \""
                                + inputPD.getIdentifier().getStringValue() + "\". Is the MimeType (\""
                                + referenceData.getMimeType() + "\") correctly set?",
                        ExceptionReport.INVALID_PARAMETER_VALUE);
                // throw new
                // ExceptionReport("Could not determine format of the input data
                // (id= \""
            ...
            if (mimeType != null) {
                ...
                if (!canUseDefault) {
                    ...
                    for (ComplexDataDescriptionType potentialFormat : formats) {
                        if (!StringUtils.isBlank(potentialFormat.getMimeType())
                                && potentialFormat.getMimeType().equalsIgnoreCase(mimeType)) {
                            if (referenceData.getSchema() != null && referenceData.getEncoding() == null) {
                                if (referenceData.getSchema().equalsIgnoreCase(potentialFormat.getSchema())) {
                                    format = potentialFormat;
                                }
                            ...
                            if (referenceData.getSchema() != null && referenceData.getEncoding() != null) {
                                if (referenceData.getSchema().equalsIgnoreCase(potentialFormat.getSchema())
                                        && referenceData.getEncoding()
                                                .equalsIgnoreCase(potentialFormat.getEncoding())) {
                                    format = potentialFormat;
                                }
                ...
                if (format == null) {
                    // throw new
                    // ExceptionReport("Could not determine intput format.
                    // Possibly multiple or none matching generators found.
                    // MimeType Set?",
                    // ExceptionReport.INVALID_PARAMETER_VALUE);
                    // TODO Review error message
                    throw new ExceptionReport(
                            "Could not determine input format because none of the supported formats match the given schema (\""
                                    + referenceData.getSchema() + "\") and encoding (\"" + referenceData.getEncoding()
        ...
        LOGGER.debug("Loading parser for: schema = \"" + schema + "\" , mimetype = \"" + mimeType + "\", encoding = \""
                + encoding + "\"");
        ...
        try {
            ...
            LOGGER.info("Looking for matching Parser ..." + " schema: \"" + schema + "\", mimeType: \"" + mimeType
                    + "\", encoding: \"" + encoding + "\"");

            parser = ParserFactory.getInstance().getParser(schema, mimeType, encoding, algorithmInputClass);

            if (parser == null) {
                throw new ExceptionReport("Error. No applicable parser found for schema=\"" + schema + "\", mimeType=\""
                        + mimeType + "\", encoding=\"" + encoding + "\"", ExceptionReport.NO_APPLICABLE_CODE);
            }
        } catch (RuntimeException e) {
            throw new ExceptionReport("Error obtaining input data", ExceptionReport.NO_APPLICABLE_CODE, e);
        }
    ...
    private Format getNonDefaultFormat(net.opengis.wps.x20.InputDescriptionType inputRefDesc,
            String dataMimeType,
            String dataSchema,
            String dataEncoding) {
        // TODO still needed?
    ...
    protected IData parseComplexValue(String formatEncoding,
            String complexValue,
            String dataMimeType,
            String formatSchema,
        ...
        if (StringUtils.isBlank(formatEncoding) || formatEncoding.equalsIgnoreCase(IOHandler.DEFAULT_ENCODING)) {
            try {
                if (!complexValueCopy.contains("xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"")) {
                    complexValueCopy = complexValueCopy.replace("xsi:schemaLocation",
            ...
            // everything that goes to this condition should be inline base64
            // data
            idata = getBase64EncodedData(complexValue, parser, dataMimeType, formatSchema);
        } else {
            throw new ExceptionReport("Unable to generate encoding " + formatEncoding,
                    ExceptionReport.NO_APPLICABLE_CODE);
        }
        return idata;
    }
    ...
    // TODO-- Needs testing
    protected IData getBase64EncodedData(String complexValue,
            IParser parser,
            String dataMimeType,
            String formatSchema) throws ExceptionReport {
        File f = null;
        String complexValueCopy = complexValue.toString();
    ...
    private void handleLiteralData(InputType input) throws ExceptionReport {
        ...
        try {
            ...
            throw new ExceptionReport("The passed parameterValue: " + parameter + " for input " + inputID
                    + " is not of type: " + xmlDataType, ExceptionReport.INVALID_PARAMETER_VALUE);
        }
        ...
        if (inputDesc.getLiteralData().isSetAllowedValues()) {
            if ((!inputDesc.getLiteralData().isSetAnyValue())) {
                ...
                if (parameterObj instanceof LiteralIntBinding || parameterObj instanceof LiteralDoubleBinding
                        || parameterObj instanceof LiteralShortBinding || parameterObj instanceof LiteralFloatBinding
                        || parameterObj instanceof LiteralLongBinding || parameterObj instanceof LiteralByteBinding) {
    ...
    private void handleLiteralData(DataInputType input) throws ExceptionReport {
        ...
        net.opengis.wps.x20.InputDescriptionType inputDesc =
                XMLBeansHelper.findInputByID(inputID, processOffering.getProcess());

        LiteralDataDomain literalDataDomain =
                ((LiteralDataType) inputDesc.getDataDescription()).getLiteralDataDomainArray(0);
    ...
    private boolean checkRange(IData parameterObj,
    ...
    private void handleComplexValueReference(InputType input) throws ExceptionReport {
        ...
        if (referenceData.isSetMimeType() && referenceData.getMimeType() != null) {
            ...
            if (format == null) {
                throw new ExceptionReport(
                        "Possibly multiple or none matching generators found for the input data with id = \""
                                ...
                                + referenceData.getMimeType() + "\") correctly set?",
                        ExceptionReport.INVALID_PARAMETER_VALUE);
                // throw new ExceptionReport("Could not determine format of the
                // input data (id= \"" +
                // inputPD.getIdentifier().getStringValue() + "\"), given the
                // mimetype \"" + referenceData.getMimeType() + "\"",
                // ExceptionReport.INVALID_PARAMETER_VALUE);
            ...
            if (mimeType != null) {
                ...
                if (format == null) {
                    // throw new ExceptionReport("Could not determine intput
        ...
        /**** PROXY *****/
        /*
         * String decodedURL = URLDecoder.decode(dataURLString); decodedURL =
         * decodedURL.replace("&amp;", "&");
         * if(decodedURL.indexOf("&BBOX")==-1){ decodedURL =
         * decodedURL.replace("BBOX", "&BBOX"); decodedURL =
         * decodedURL.replace("outputFormat", "&outputFormat"); decodedURL =
         * decodedURL.replace("SRS", "&SRS"); decodedURL =
         * decodedURL.replace("REQUEST", "&REQUEST"); decodedURL =
         * decodedURL.replace("VERSION", "&VERSION"); decodedURL =
         * decodedURL.replace("SERVICE", "&SERVICE"); decodedURL =
         * decodedURL.replace("format", "&format"); }
         */
        ...
        // !dataURLString.toUpperCase().contains("OUTPUTFORMAT=GML2")){
        // //make sure we get GML2
        // dataURLString = dataURLString+"&outputFormat=GML2";
        // }
        // if(parser instanceof GML3BasicParser &&
        // !dataURLString.toUpperCase().contains("OUTPUTFORMAT=GML3")){
    ...
    private void handleBBoxValue(InputType input) throws ExceptionReport {
        ...
        List<IData> resultList = inputData.get(input.getIdentifier().getStringValue());
        if (resultList == null) {
            inputData.put(input.getIdentifier().getStringValue(), resultList = new ArrayList<IData>(1));
        }
    ...
    private IData parseBoundingBox(BoundingBoxType bbt) throws ExceptionReport {
        ...
        if (dim != null && (dim.compareTo(INT_MAX) > 0 || dim.compareTo(INT_MIN) < 0)) {
            throw new ExceptionReport(
                    String.format("Unsupported BoundingBox dimension %s. Has to be betweeen %s and %s!", dim, INT_MIN,
                            INT_MAX),
                    ExceptionReport.INVALID_PARAMETER_VALUE);
        }
     ...
     *            The client input
     */
    private void handleBBoxValue(DataInputType input) throws ExceptionReport {
        ...
        try {
            ...
            LOGGER.error("XmlException occurred while trying to parse bounding box: "
                    + (boundingBoxDocument == null ? null : boundingBoxDocument.toString()), e);
        }
        ...
        if (resultList == null) {
            inputData.put(input.getId(), resultList = new ArrayList<IData>(1));
        }
        resultList.add(envelope);

    }

    private IData parseBoundingBox(net.opengis.ows.x20.BoundingBoxType bbt) throws ExceptionReport {
        final BigInteger dim = bbt.getDimensions();
        final double[] lower;
        final double[] upper;
    ...
    // private InputStream retrievingZippedContent(URLConnection conn) throws
    // IOException{
