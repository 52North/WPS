...
public class WebProcessingService implements ServletContextAware, ServletConfigAware {
     ...
     */
    private static OutputStream getConfiguredOutputStream(HttpServletRequest hsRequest,
            HttpServletResponse hsResponse) throws IOException {
        /*
         * Forbids clients to cache the response May solve problems with proxies
    ...
    public void init() {
        ...
        // this is important to set the lon lat support for correct CRS
        // transformation.
        ...
        Map<String, ConfigurationModule> parserMap = WPSConfig.getInstance().getConfigurationManager()
                .getConfigurationServices().getActiveConfigurationModulesByCategory(ConfigurationCategory.PARSER);
        ParserFactory.initialize(parserMap);
        LOGGER.info("Initialized {}", ParserFactory.getInstance());

        Map<String, ConfigurationModule> generatorMap = WPSConfig.getInstance().getConfigurationManager()
                .getConfigurationServices().getActiveConfigurationModulesByCategory(ConfigurationCategory.GENERATOR);
        ...
        try {
            String capsConfigPath = getApplicationBaseDir() + File.separator + PUBLIC_CONFIG_FILE_DIR + File.separator
        ...
        LOGGER.info("Service base url is {} | Service endpoint is {}", conf.getServiceBaseUrl(),
    ...
    protected void doGet(HttpServletRequest req,
        ...
        try {
            OutputStream out = res.getOutputStream(); // closed by
                                                      // res.flushBuffer();
    ...
    protected void doPost(HttpServletRequest req,
        ...
        try {
            ...
            if (characterEncoding == null || characterEncoding.length() == 0) {
                characterEncoding = "UTF-8"; // default character encoding if
                                             // unspecified
            }
            ...
            if (contentLength > MAXIMUM_REQUEST_SIZE) {
                ...
                ExceptionReport er =
                        new ExceptionReport("Request body too large, limited to " + MAXIMUM_REQUEST_SIZE + " bytes",
            ...
            if (contentType != null && contentType.startsWith("application/x-www-form-urlencoded")) {
                if (documentString.startsWith(SPECIAL_XML_POST_VARIABLE + "=")) {
                    // This is a hack to permit xml to be easily submitted via a
                    // form POST.
                    // By convention, we are allowing users to post xml if they
                    // name it
    ...
    private static void handleException(ExceptionReport exception,
        ...
        try {
            ...
            exception.getExceptionDocument(version).save(res.getOutputStream(), XMLBeansHelper.getXmlOptions());
