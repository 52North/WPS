...
public class RepositoryManager {
	...
	private ProcessIDRegistry globalProcessIDs = ProcessIDRegistry
			.getInstance();
	...
	private void loadAllRepositories() {
		...
		Repository[] repositoryList = WPSConfig.getInstance()
		...
		for (Repository repository : repositoryList) {
			...
			try {
				...
				Class repositoryClass = RepositoryManager.class
						.getClassLoader().loadClass(repositoryClassName);
				...
				for (Constructor constructor : constructors) {
					...
					if (constructor.getParameterTypes().length == 1
						...
						Property formatProperty = WPSConfig.getInstance()
				...
				// (IAlgorithmRepository)RepositoryManager.class.getClassLoader().loadClass(repositoryClassName).getMethod("getInstance",
				...
				e.printStackTrace();
				LOGGER.warn("An error occured while registering AlgorithmRepository: "
						+ repositoryClassName + ". Reason " + e.getMessage());
	...
	/**
	 * Methods looks for Algorithhm in all Repositories. The first match is
	...
	public List<String> getAlgorithms() {
		...
		for (IAlgorithmRepository repository : repositories) {
			if (repository.getAlgorithmNames() != null)
				allAlgorithmNamesCollection.addAll(repository
						.getAlgorithmNames());
		}
	...
	public List<String> getDatas() {
		List<String> allDataNamesCollection = new ArrayList<String>();
		for (IAlgorithmRepository repository : repositories) {
			if (repository instanceof DefaultTransactionalProcessRepository) {
				DefaultTransactionalProcessRepository defRepository = (DefaultTransactionalProcessRepository) repository;
				if (defRepository.getDataNames() != null)
					allDataNamesCollection.addAll(defRepository.getDataNames());
			}
		}
		return allDataNamesCollection;
	}
	...
	public boolean containsData(String dataName) {
		...
		for (IAlgorithmRepository repositoryg : repositories) {
			DefaultTransactionalProcessRepository repository;
			if (repositoryg instanceof DefaultTransactionalProcessRepository) {
				repository = (DefaultTransactionalProcessRepository) repositoryg;
				if (repository.containsData(dataName)) {
					return true;
				}
			}
		}
	...
	public IAlgorithmRepository getRepositoryForClassName(String className) {
		...
		for (IAlgorithmRepository repository : repositories) {
			...
			LOGGER.info("getRepositoryForClassName - current:"
	...
	public IAlgorithmRepository getRepositoryForSchema(String schema) {
		LOGGER.info("getRepositoryForSchema:" + schema);
		for (IAlgorithmRepository repository : repositories) {
			if (repository instanceof DefaultTransactionalProcessRepository) {
				LOGGER.info(schema
						+ " equals "
	...
	public DataDescriptionType getDataDescription(String dataName) {
		LOGGER.info("getDataDescription:" + dataName);
		for (IAlgorithmRepository repositoryg : repositories) {
			DefaultTransactionalProcessRepository repository;
			if (repositoryg instanceof DefaultTransactionalProcessRepository) {
				repository = (DefaultTransactionalProcessRepository) repositoryg;
				if (repository.containsData(dataName)) {
					return repository.getDataDescription(dataName);
				}
			}
		}
		return null;
	...
	public DefaultTransactionalDataRepository getRepositoryForData(
			String dataName) {
		LOGGER.info("getDataDescription:" + dataName);
		for (IAlgorithmRepository repositoryg : repositories) {
			DefaultTransactionalProcessRepository repository;
			if (repositoryg instanceof DefaultTransactionalProcessRepository) {
				repository = (DefaultTransactionalProcessRepository) repositoryg;
				if (repository.containsData(dataName)) {
					return new DefaultTransactionalDataRepository(
